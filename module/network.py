...
class NeuralNetwork:
    ...
    def save(self, name = None):
        ...
        with open('E:/Tai_lieu/AIGameProject/AIGameProject/SavedNetwork/' + name + '_weights.json', 'w') as f:
            json.dump(dumped, f)
        dumped = json.dumps(self.biases, cls=NumpyEncoder)
        with open('E:/Tai_lieu/AIGameProject/AIGameProject/SavedNetwork/' + name + '_biases.json', 'w') as f:
            json.dump(dumped, f)
        """
    ...
    def render(self, window, vision, startPos):
        neuralRadius = 10
        neuralRadiusFirstLayer = 10
        lineWidth = 2
        marginNeuralFirstLayer = 15
        marginNeural = 15
        marginLayer = 100
        """
        ...
        for i in range(len(network)):  # for each layer
            ...
            for j in range(len(network[i][0])):  # for each neuron in current layer
                ...
                if i == 0:
                    if j < 8:
                        intensity = abs(int(network[i][0][j] / 20 * 255))  # neuron intensity
                    else:
                        intensity = abs(int(network[i][0][j] / 40 * 255))
                else:
                    intensity = abs(int(network[i][0][j] * 255))
                if intensity > 255:
                    intensity = 255
                elif intensity < 0:
                    intensity = 0
